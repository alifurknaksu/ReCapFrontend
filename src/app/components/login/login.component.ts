    ...
    if (this.loginForm.valid) {
      ...
      this.authService.login(this.userForLoginDto).subscribe(
        (response) => {
          ...
          this.toastrService.success(response.message, 'Login');
          window.location.reload();
        },
        (responseError) => {
          this.toastrService.error(responseError.error.message, 'Login');
        }
      );
    } else {
      this.toastrService.error('Please,fill in the blanks', 'Login');
    }
  }
